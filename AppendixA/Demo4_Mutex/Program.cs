...
#region Using Interlocked class
//class Account
//{
//    // public decimal Balance { get; set; }
...
class Account
{
    ...
    private bool _lockBalance;
    public void Credit(decimal amount)
    {
        _lockBalance = _mutex.WaitOne();
        if (_lockBalance)
        {
            try
            {
            ...
            {
                if (_lockBalance) _mutex.ReleaseMutex();
            }
        }
        else
        {
            WriteLine($"Could not credit: {amount}");
        }
    }
    public void Debit(decimal amount)
    {
        ...
        {
            WriteLine($"Could not debit: {amount}");
        }
    }
}
#endregion 
